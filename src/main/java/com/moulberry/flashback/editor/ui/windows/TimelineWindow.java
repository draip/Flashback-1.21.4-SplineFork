...
import com.moulberry.flashback.vectors.VectorHandleManager;
...
import java.util.*;
                    ...
                    public class TimelineWindow {
                        ...
                        public static List<VectorHandle> currentHandles = null;


                        private enum StickySelectionMode { NONE, BLUE, RED }
                        @Nullable
                        private static SelectedKeyframes stickyAdvancedCameraSelection = null;
                        private static StickySelectionMode stickyAdvancedCameraMode = StickySelectionMode.NONE;

                        public static @Nullable SelectedKeyframes getStickyAdvancedCameraSelection() {
                            return stickyAdvancedCameraSelection;
                        }
                        public static boolean isStickySelectionEditMode() {
                            return stickyAdvancedCameraMode == StickySelectionMode.RED;
                        }
                        ...
                        private static void handleClick(ReplayServer replayServer, int totalTicks, float contentY) {
                            ...
                            // Controls (skip, pause, etc.)
                            if (hoveredControls) {
                                if (hoveredSkipBackwards) {
                                    FlashbackMeta meta = replayServer.getMetadata();
                                    Integer lastMarker = meta.replayMarkers.floorKey(cursorTicks - 1);
                                    replayServer.goToReplayTick(lastMarker != null ? lastMarker : 0);
                                    return;
                                }
                                if (hoveredSlowDown) {
                                ...
                                if (hoveredSkipForwards) {
                                    FlashbackMeta meta = replayServer.getMetadata();
                                    Integer nextMarker = meta.replayMarkers.ceilingKey(cursorTicks + 1);
                                    replayServer.goToReplayTick(nextMarker != null ? nextMarker : replayServer.getTotalReplayTicks());
                                    return;
                                }
                            }

                            // Timeline area (handle export bar/zoom bar)
                            ...
                            // Tracks/keyframes selection with sticky logic
                            boolean clickedOnKeyframe = false;
                            if (mouseY > y + middleY && mouseY < y + height && mouseX > x + middleX && mouseX < x + width) {
                                ...
                                if (trackIndex >= 0 && trackIndex < editorScene.keyframeTracks.size()) {
                                    ...
                                    if (closest != null) {
                                        ...
                                        int clickedTick = closest.getKey();
                                        Keyframe clickedKeyframe = closest.getValue();
                                        boolean isAdvancedCamera = clickedKeyframe instanceof CameraKeyframe &&
                                                clickedKeyframe.interpolationType() == InterpolationType.ADVANCED;

                                        // LEFT CLICK: sticky blue selection for advanced camera keyframes (acts like normal selection, draggable)
                                        if (leftClicked && isAdvancedCamera) {
                                            boolean wasBlue =
                                                    stickyAdvancedCameraSelection != null &&
                                                            stickyAdvancedCameraMode == StickySelectionMode.BLUE &&
                                                            stickyAdvancedCameraSelection.trackIndex() == trackIndex &&
                                                            stickyAdvancedCameraSelection.keyframeTicks().contains(clickedTick);

                                            if (wasBlue) {
                                                stickyAdvancedCameraSelection = null;
                                                stickyAdvancedCameraMode = StickySelectionMode.NONE;
                                                selectedKeyframesList.clear();
                                                currentHandles = null;
                                            } else {
                                                IntSet intSet = new IntOpenHashSet();
                                                intSet.add(clickedTick);
                                                stickyAdvancedCameraSelection = new SelectedKeyframes(keyframeTrack.keyframeType, trackIndex, intSet);
                                                stickyAdvancedCameraMode = StickySelectionMode.BLUE;
                                                selectedKeyframesList.clear();
                                                selectedKeyframesList.add(stickyAdvancedCameraSelection);
                                                grabbedKeyframe = true;
                                                grabbedKeyframeMouseX = mouseX;
                                                grabbedKeyframeTick = clickedTick;
                                                grabbedKeyframeTrack = trackIndex;
                                                enableKeyframeMovement = false;

                                                currentHandles = VectorHandleManager.createHandlesForKeyframe(keyframeTrack, clickedTick, trackIndex);                                            }
                                            return;
                                        }

                                        // RIGHT CLICK: sticky red selection for advanced camera keyframes, open popup
                                        if (rightClicked && isAdvancedCamera) {
                                            boolean wasRed =
                                                    stickyAdvancedCameraSelection != null &&
                                                            stickyAdvancedCameraMode == StickySelectionMode.RED &&
                                                            stickyAdvancedCameraSelection.trackIndex() == trackIndex &&
                                                            stickyAdvancedCameraSelection.keyframeTicks().contains(clickedTick);
                                            ...
                                            if (wasRed) {
                                                ...
                                                editingKeyframeTick = clickedTick;
                                                grabbedKeyframeTrack = trackIndex;
                                            }
                                            return;
                                        }

                                        // If we click a different keyframe or non-advanced, clear sticky
                                        stickyAdvancedCameraSelection = null;
                                        stickyAdvancedCameraMode = StickySelectionMode.NONE;

                                        // Regular keyframe selection logic
                                        ...
                                        for (SelectedKeyframes selectedKeyframes : oldSelectedKeyframesList) {
                                            if (selectedKeyframes.trackIndex() == trackIndex) {
                                                reuseOld = selectedKeyframes.keyframeTicks().contains(clickedTick);
                                                break;
                                            }
                                        }
                                        ...
                                        if (ReplayUI.isCtrlOrCmdDown()) {
                                            ...
                                            for (SelectedKeyframes selectedKeyframes : selectedKeyframesList) {
                                                if (selectedKeyframes.type() == keyframeTrack.keyframeType && selectedKeyframes.trackIndex() == trackIndex) {
                                                    if (selectedKeyframes.keyframeTicks().remove(clickedTick)) {
                                                        deselectedClicked = true;
                                                    } else {
                                                        selectedKeyframes.keyframeTicks().add(clickedTick);
                                                    }
                                                    handled = true;
                                                    break;
                                                }
                                        ...
                                        if (!deselectedClicked) {
                                            if (leftClicked) {
                                                if (ImGui.isMouseDoubleClicked(ImGuiMouseButton.Left)) {
                                                    ...
                                                    if (clickedKeyframe.keyframeType().supportsHandler(minecraftKeyframeHandler)) {
                                                        clickedKeyframe.createChange().apply(minecraftKeyframeHandler);
                                                    }
                                                } else {
                                                    grabbedKeyframe = true;
                                                    grabbedKeyframeMouseX = mouseX;
                        ...
                        private static void renderKeyframes(float x, float y, float mouseX, int minTicks, float availableTicks, int totalTicks) {
                            ...
                            for (int trackIndex = 0; trackIndex < editorScene.keyframeTracks.size(); trackIndex++) {
                                ...
                                for (int tick = minTicks - 10; tick <= minTicks + availableTicks + 10; tick++) {
                                    ...
                                    boolean isSticky = stickyAdvancedCameraSelection != null &&
                                            stickyAdvancedCameraSelection.trackIndex() == trackIndex &&
                                            stickyAdvancedCameraSelection.keyframeTicks().contains(tick);
                                    boolean isAdvancedCamera = keyframe instanceof CameraKeyframe &&
                                            keyframe.interpolationType() == InterpolationType.ADVANCED;

                                    // Always prioritize sticky mode color for advanced camera keyframes
                                    if (isSticky && isAdvancedCamera) {
                                        int newTick = tick;
                                        ...
                                        switch (stickyAdvancedCameraMode) {
                                            case BLUE -> drawKeyframe(drawList, keyframe.interpolationType(), midX, midY, 0xFFFF0000); // Blue mode shows red (left click = red)
                                            case RED -> drawKeyframe(drawList, keyframe.interpolationType(), midX, midY, 0xFF0000FF);  // Red mode shows blue (right click = blue)
                                            default -> {
                                                // Optionally: draw nothing or a default color
                                            }
                                        }
                                    } else if (selectedKeyframesForTrack != null && selectedKeyframesForTrack.keyframeTicks().contains(tick)) {
                                        int newTick = tick;
                                        if (grabMovementInfo != null) {
                                            newTick = tick + grabMovementInfo.grabbedDelta;
                                            if (grabMovementInfo.grabbedScalePivotTick >= 0) {
                                    ...
                                    // Timelapse duration display (unchanged)
                                    if ((selectedKeyframesForTrack == null || grabMovementInfo == null) && keyframeTrack.keyframeType == TimelapseKeyframeType.INSTANCE) {
                                        ...
                                        if (floorEntry != null) {
                                            ...
                                            if (textWidth <= rightX - leftX) {
                                                ImGui.getWindowDrawList().addText(x + midX - textWidth / 2, textY, textColour, "Duration: " + message);
                                            } else {
                                                textWidth = ImGuiHelper.calcTextWidth(message);
                                                if (textWidth <= rightX - leftX) {
                                                    ImGui.getWindowDrawList().addText(x + midX - textWidth / 2, textY, textColour, message);
                                                }
                                            }
    ...
    private static void drawKeyframe(ImDrawList drawList, InterpolationType interpolationType, float x, float y, int colour) {
        ...
        switch (interpolationType) {
            ...
            case ADVANCED -> {
                // Outer circle
                drawList.addCircleFilled(x, y, keyframeSize, colour);
                // Inner smaller circle (hollow effect)
                drawList.addCircleFilled(x, y, keyframeSize / 2.5f, ImGui.getColorU32(0, 0, 0, 1)); // Use black or a transparent color
            }
        }
    }
